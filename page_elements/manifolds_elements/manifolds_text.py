...
and the phase space can be represented as a direct sum of eigenspaces, $E_i(x_*)$, corresponding to the eigenvalues, $\lambda_i$, of the matix $A$.
...
Let us call the eigenspace (??? sum of eigenspaces with $Re(\lambda_i)<0$ ???) in which trajectories converge to the origin going forwards in time a stable eigenspace, $E_s(x_*):=\{x_0: x_0 e^{At} \to 0,$ as $t \to \infty\}$,
...
You can now switch to the initial system (the linearisation of which we were exploring so far) by chaging "System" radio button. Try several initial conditions.
...
with a property that tragectories within those sets $\textbf{\textit{converge forwards/backwards in time to the origin}}$
...
The manifolds can be made $\textbf{\textit{invariant}}$ by extending them under the flow $\phi_t$ of a given dynamical system, i.e
'''
